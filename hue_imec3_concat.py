...
data_folder = 'H:/AE4/004/AE4_u004_010/'
analyzer_path = glob.glob(data_folder + '*.analyzer')[0]
spikesort = 'kilosort'
fs = 30000
start_sample = 368327460
...
if spikesort == 'jrclust':
    ...
    clu_labels = pd.read_csv(kilosort_folder + 'cluster_groups.csv', sep='\t')
...
for cluster in np.sort(spikes.cluster.unique())[np.sort(spikes.cluster.unique()) > 0]:
    if clu_labels[clu_labels['cluster_id'] == cluster].group.values[0] != 'noise':
        ...
        trial_num_plot = trial_num[(trial_num.ori == orientation_max) & (trial_num.s_freq == s_freq_max)]
        # trial_num_plot = trial_num
        ...
        if make_plot == 'y':
            ...
            ax.scatter(trial_num_plot.groupby('colormod').mean()[cluster_str].index.values,
                       trial_num_plot.groupby('colormod').mean()[cluster_str].values,
                       color=color_hex, s=100, zorder=2)
            ax.fill_between(trial_num_plot.groupby('colormod').mean()[cluster_str].index.values,
                            trial_num_plot.groupby('colormod').mean()[cluster_str].values -
                            trial_num_plot.groupby('colormod').sem()[cluster_str].values,
                            trial_num_plot.groupby('colormod').mean()[cluster_str].values +
                            trial_num_plot.groupby('colormod').sem()[cluster_str].values,
